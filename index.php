...
// বেশি পোস্টকারীদের গুরুত্ব দেওয়ার জন্য PHP ওয়েটেড অ্যালগরিদম
function weightedRandomShufflePHP($postsList) {
    $authorCounts = [];
    foreach ($postsList as $post) {
        $author = isset($post['metadata']['author']) ? $post['metadata']['author'] : 'default';
        $authorCounts[$author] = ($authorCounts[$author] ?? 0) + 1;
    }

    usort($postsList, function($a, $b) use ($authorCounts) {
        $authorA = $a['metadata']['author'] ?? 'default';
        $authorB = $b['metadata']['author'] ?? 'default';
        
        $weightA = pow($authorCounts[$authorA], 1.5) * (mt_rand(1, 100) / 100);
        $weightB = pow($authorCounts[$authorB], 1.5) * (mt_rand(1, 100) / 100);
      ...
      <?php else: ?>
        <?php
          ...
          if (is_array($files) && !isset($files['message'])):
              ...
              // র‍্যান্ডম প্রpriority অ্যালগরিদম রান করানো
              $finalPosts = weightedRandomShufflePHP($parsedPosts);

              foreach ($finalPosts as $item):
                  $file = $item['file'];
                  $meta = $item['metadata'];
                  $body = $item['body'];
